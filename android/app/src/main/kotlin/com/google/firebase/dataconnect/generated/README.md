...
is the query named "GetMyReviews",
which can be accessed via the [ExampleConnector.getMyReviews] property.
...
For example, the "GetMyReviews" query has no variables
and can be executed via the
[ExampleConnector.getMyReviews]
property as follows:

```kotlin
val connector = ExampleConnector.instance
val queryResult = connector.getMyReviews.execute()
println("GetMyReviews query returned: ${queryResult.data}")
```
...
For example, the "SeedDemoData" mutation has no variables
and can be executed via the
[ExampleConnector.seedDemoData]
property as follows:

```kotlin
val connector = ExampleConnector.instance
val mutationResult = connector.seedDemoData.execute()
println("SeedDemoData mutation returned: ${mutationResult.data}")
```
...
For example, the "CreateReview" mutation has 4 required variables ("movieId", "rating", "review", and "watchId")
and can be executed via the [ExampleConnector.createReview]
property as follows:

```kotlin
val connector = ExampleConnector.instance
val mutationResult = connector.createReview.execute(movieId=java.util.UUID.randomUUID(), rating=7324, review="garply", watchId=java.util.UUID.randomUUID())
